...
def test_events_tool_call():
    msg = AIMessage(
        content="",
        tool_calls=[{"name": "read_file", "args": {"file_path": "/x"}, "id": "1", "type": "tool_call"}],
    )
    events = list(Harness._events_for_message(msg))
    assert events == [("tool_call", "read_file", {"file_path": "/x"})]
...
def test_events_tool_result():
    msg = ToolMessage(content="OK", tool_call_id="1", name="write_file")
    assert list(Harness._events_for_message(msg)) == [("tool_result", "write_file", "OK")]
